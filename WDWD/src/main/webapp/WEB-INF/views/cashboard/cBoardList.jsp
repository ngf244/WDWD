				...
				$.ajax({
					...
					success: function(list){
						...
						for(var i in list) {
							...
							var $input = $('<input type="hidden" name="boNum" value="' + list[i].boNum + '">');
							
							var $div1 = $('<div class="boardImg">');
							var $img = $('<img src="${ contextPath }/resources/images/emptyImage.png">'); // if 문 추가
							...
							if(list[i].thumbnail != undefined) {
								$img.attr("src", '${ contextPath }/resources/real_photo/' + list[i].thumbnail)
							}
							
							if(list[i].boCategory == 1) {
								$div5.text('컨텐츠 제작');
							} else if(list[i].boCategory == 2) {
								$div5.text('컨텐츠 수정');
							} else {
								$div5.text('기타');
							}
							
							$div6.text(list[i].boTitle);
							$div7.html('의뢰인 : <span class="smallOption">' + list[i].boWriter + '</span>');
							
							$('#removeImg').html(list[i].boContent);
							$('#removeImg').find('img').remove();
							$div8.html($('#removeImg').html());
							
							if(list[i].cbStep == 1) {
								$div9.text('참여인원 : ' + list[i].boReNum + ' 명');
							} else {
								$div9.text('참여인원 : 마감');
							}
							...
							if(list[i].cbDate == 0) {
								$div10.text('기간 : 제한 없음');
							} else if(list[i].cbDate == '마감') {
								$div10.text('기간 : 마감');
							} else {
								var date = new Date(list[i].cbDate);
							...
							if(list[i].cbCash == 0) {
								...
								$div11.text('의뢰비 : ' + list[i].cbCash + ' CASH');
							}
